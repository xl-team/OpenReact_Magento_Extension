...
class React_React_Block_Login extends Mage_Core_Block_Template
{
	...
	protected function _getRefererUrl()
	{
		...
		if ($url = $this->getRequest()->getParam('referer_url'))
			$refererUrl = $url;
		else if ($url = $this->getRequest()->getParam('r64'))
			$refererUrl = Mage::helper('core')->urlDecode($url);
		else if ($url = $this->getRequest()->getParam('uenc'))
			$refererUrl = Mage::helper('core')->urlDecode($url);
		else
	...
	public function _isUrlInternal($url)
	{
		...
		{
			if (
				(strpos($url, Mage::app()->getStore()->getBaseUrl()) === 0) || (strpos($url, Mage::app()->getStore()->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK, true)) === 0)
			)
			{
				return true;
			}
