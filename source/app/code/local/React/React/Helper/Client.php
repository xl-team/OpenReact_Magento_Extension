...
class React_React_Helper_Client extends Mage_Core_Helper_Abstract
{
	...
	*/
	protected function _sendRequest($request)
	{
		...
		if (!$fp)
			throw new React_React_Exception('Failed to connect to endpoint `%s`.', array($this->_endpoint));
	...
	*/
	protected function _processResponse($response)
	{
		...
		if (!isset($headers['Content-Type']) || 0 !== strpos($headers['Content-Type'], 'text/xml'))
			throw new React_React_Exception('Missing required Content-Type text/xml.');
		...
		{
			throw new React_React_Exception('Could not parse body `%s` as XML.', array($body), $e);
		}
		...
		if (isset($responseXml->fault))
		{
			...
			throw new React_React_Exception('Response Fault (%s): %s ', array($code, $message), null, (int)$code);
		}
	...
	protected function _encodeParam(SimpleXmlElement $param, $value)
	{
		...
		switch (gettype($value))
		{
			...
			case 'integer':
				...
				break;
			default:
				throw new React_React_Exception('Cannot convert unsupported PHP value type `%s` to a XML-RPC value type.', array($child->getName()));
		}
	}
	...
	protected function _decodeParam(SimpleXmlElement $param)
	{
		...
		switch ($child->getName())
		{
			...
			case 'string':
				return (string)$child;
			default:
				throw new React_React_Exception('Unsupported XML-RPC value type `%s` found.', array($child->getName()));
		}
	}
}